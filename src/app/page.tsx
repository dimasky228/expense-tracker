...
const featureIcons = [
  <svg key="bolt" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="h-6 w-6" aria-hidden="true">
    <path strokeLinecap="round" strokeLinejoin="round" d="m3.75 13.5 10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z" />
  </svg>,
  <svg key="chart" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="h-6 w-6" aria-hidden="true">
    <path strokeLinecap="round" strokeLinejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z" />
  </svg>,
  <svg key="lock" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="h-6 w-6" aria-hidden="true">
    <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 1 0-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25H6.75a2.25 2.25 0 0 0-2.25 2.25v6.75a2.25 2.25 0 0 0 2.25 2.25Z" />
  </svg>,
];

const mockTransactions = [
  { name: "Netflix", category: "Entertainment", amount: "-$15.99", recurring: true, positive: false },
  { name: "Whole Foods", category: "Groceries", amount: "-$84.20", recurring: false, positive: false },
  { name: "Venmo from Alex", category: "Transfer", amount: "+$120.00", recurring: false, positive: true },
  { name: "Spotify", category: "Entertainment", amount: "-$9.99", recurring: true, positive: false },
];
...
export default async function Home() {
  ...
  const features = [
    { title: t("feature1Title"), description: t("feature1Desc"), icon: featureIcons[0] },
    { title: t("feature2Title"), description: t("feature2Desc"), icon: featureIcons[1] },
    { title: t("feature3Title"), description: t("feature3Desc"), icon: featureIcons[2] },
  ];
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-50">
      ...
      <main>
        ...
        >
          ...
          <div className="relative mx-auto flex max-w-6xl flex-col items-center gap-16 lg:flex-row lg:items-center lg:gap-12">
            <div className="flex-1 text-center lg:text-left">
              ...
              >
                {t("heroTitle").split("understands").length > 1 ? (
                  <>
                    {t("heroTitle").split("understands")[0]}
                    <span className="text-cyan-400">understands</span>
                    {t("heroTitle").split("understands")[1]}
                  </>
                ) : (
                  t("heroTitle")
                )}
              ...
              <div className="animate-float w-full max-w-sm rounded-2xl border border-zinc-700/50 bg-zinc-900 p-5 shadow-2xl shadow-black/60">
                ...
                <div className="space-y-3.5">
                  {mockTransactions.map((tr) => (
                    <div key={tr.name} className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <p className="truncate text-sm font-medium text-zinc-200">{tr.name}</p>
                        <div className="flex flex-wrap items-center gap-1.5">
                          <span className="text-xs text-zinc-500">{tr.category}</span>
                          {tr.recurring && (
                      ...
                      <span className={`shrink-0 text-sm font-semibold tabular-nums ${tr.positive ? "text-emerald-400" : "text-red-400"}`}>
                        {tr.amount}
                      </span>
                    </div>
                  ))}
        ...
        {/* ── Features ── */}
        <section aria-labelledby="features-heading" className="px-6 py-24 md:py-32">
          <div className="mx-auto max-w-6xl">
            <div className="mb-16 text-center">
              ...
              <p className="mt-4 text-lg text-zinc-400">{t("featuresSubtitle")}</p>
            </div>
            <div className="grid gap-6 md:grid-cols-3">
              {features.map((f) => (
                ...
                >
                  ...
                  <h3 className="mb-2 text-xl font-semibold text-zinc-50">{f.title}</h3>
                  <p className="leading-relaxed text-zinc-400">{f.description}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        >
          <div className="mx-auto max-w-6xl">
            <div className="mb-16 text-center">
              <h2 id="how-heading" className="text-3xl font-bold tracking-tight text-zinc-50 sm:text-4xl">
                {t("howTitle")}
              </h2>
              <p className="mt-4 text-lg text-zinc-400">{t("howSubtitle")}</p>
            </div>
            <div className="grid gap-10 md:grid-cols-3">
              {steps.map((step) => (
                <div key={step.number} className="flex flex-col items-start">
                  ...
                  <h3 className="mb-2 text-xl font-semibold text-zinc-50">{step.title}</h3>
        ...
        {/* ── Pricing ── */}
        <section aria-labelledby="pricing-heading" className="border-t border-zinc-800/50 px-6 py-24 md:py-32">
          <div className="mx-auto max-w-6xl">
            <div className="mb-16 text-center">
              <h2 id="pricing-heading" className="text-3xl font-bold tracking-tight text-zinc-50 sm:text-4xl">
                {t("pricingTitle")}
              </h2>
              <p className="mt-4 text-lg text-zinc-400">{t("pricingSubtitle")}</p>
            </div>
            <div className="mx-auto grid max-w-3xl gap-6 md:grid-cols-2">
              ...
              <div className="flex flex-col rounded-2xl border border-zinc-800/60 bg-zinc-900/50 p-8">
                ...
                <div className="mb-1 flex items-end gap-1">
                  <span className="text-5xl font-bold text-zinc-50">{t("freePrice")}</span>
                </div>
                <p className="mb-8 text-sm text-zinc-500">{t("freePeriod")}</p>
                <ul className="mb-8 flex-1 space-y-3 text-sm text-zinc-400">
                  {[t("freeF1"), t("freeF2"), t("freeF3")].map((item) => (
                    <li key={item} className="flex items-center gap-2">
                      <span className="text-cyan-400" aria-hidden="true">✓</span>
                      {item}
                    </li>
                  ))}
                  {[t("freeX1"), t("freeX2"), t("freeX3")].map((item) => (
              ...
              <div className="relative flex flex-col rounded-2xl border border-cyan-400/30 bg-zinc-900/50 p-8">
                ...
                <div className="mb-1 flex items-end gap-1">
                  <span className="text-5xl font-bold text-zinc-50">{t("proPrice")}</span>
                  <span className="mb-1.5 text-zinc-400">/mo</span>
                </div>
                <p className="mb-8 text-sm text-zinc-500">{t("proPeriod")}</p>
                <ul className="mb-8 flex-1 space-y-3 text-sm text-zinc-400">
                  {[t("proF1"), t("proF2"), t("proF3"), t("proF4"), t("proF5"), t("proF6")].map((item) => (
                    <li key={item} className="flex items-center gap-2">
                      <span className="text-cyan-400" aria-hidden="true">✓</span>
                      {item}
                    </li>
            ...
            <div className="space-y-2">
              {faqs.map((faq) => (
                ...
                >
                  ...
                  <div className="faq-body px-6 pb-5 text-zinc-400 leading-relaxed">{faq.a}</div>
                </details>
              ))}
            </div>
          </div>
